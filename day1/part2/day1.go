...
var revNumberMap = map[string]string{
	"eno":   "1",
	"owt":   "2",
	"eerht": "3",
	"ruof":  "4",
...
func main() {
	...
	for _, l := range lines {
		...
		re := regexp.MustCompile(`\d|one|two|three|four|five|six|seven|eight|nine`) //|[one|two|three|four|five|six|seven|eight|nine]]`)
		firstDigitRaw := re.FindString(l)
		translatedFirst, ok := numberMap[firstDigitRaw]
		if !ok {
			translatedFirst = firstDigitRaw
		}
		...
		rre := regexp.MustCompile(`\d|eno|owt|eerht|ruof|evif|xis|neves|thgie|enin`) //|[one|two|three|four|five|six|seven|eight|nine]]`)
		lastDigitRaw := rre.FindString(reversedString)

		translatedLast, ok := revNumberMap[lastDigitRaw]
		if !ok {
			translatedLast = lastDigitRaw
		}

		fmt.Println(translatedFirst, translatedLast, l, reversedString)

		lineNumberStr := translatedFirst + translatedLast
		lineNumber, err := strconv.Atoi(lineNumberStr)
		if err != nil {
			fmt.Println(lineNumberStr, translatedFirst, translatedLast, l)
			panic(err)
		}
		total += lineNumber
		fmt.Println(total)
