const input:string = `.......855.442......190..................................969..........520.......59.............................................172..........
...
const getFullNumber = (l:number, c:number):number => {
  const initial = lines[l][c]
  let result = initial;
  ...
  while(true) { 
    forthIndex++; 
    if(forthIndex == lines.length) break;

    let next = lines[l][forthIndex]; 
  ...
  while(true) { 
    ...
    let prev = lines[l][backIndex]; 
    let prevNumber = prev.match(/\d/)
    
    if(!prevNumber) break;
    result = prevNumber[0] + result;
  }

  const i = parseInt(result)
  if(isNaN(i)) {
    console.log(result, initial, l, c)
  }
  return parseInt(result);
}

const collectAdjacent = (l:number, c:number):number[] => {
  //top row
  const top = l - 1;
  const bottom = l + 1;
  const left = c - 1;
  ...
  if( top > -1 ) {
    const tc = lines[top][c].match(/\d/)
    let tr:RegExpMatchArray|null = null;
    let tl:RegExpMatchArray|null = null;

    if( left > -1 ) {
      tl = lines[top][left].match(/\d/)
    }
    if( right < lines.length ) {
      tr = lines[top][right].match(/\d/)
    }

    if( tc ) {
      numbers.push(getFullNumber(top, c))
    }
    else {
      if( tl ) {
        numbers.push(getFullNumber(top,left))
      }
      if( tr ) {
        numbers.push(getFullNumber(top,right))
      }
    }
  }
  
  // middle
  let cr:RegExpMatchArray|null = null;
  let cl:RegExpMatchArray|null = null;

  if( left > -1 ) {
    cl = lines[l][left].match(/\d/)
  }
  if( right < lines.length) {
    cr = lines[l][right].match(/\d/)
  }
  if( cl ) {
    numbers.push(getFullNumber(l,left))
  }
  if( cr ) {
    numbers.push(getFullNumber(l,right))
  }

  //bottom
  if( bottom < lines.length ) {
    const bc = lines[bottom][c].match(/\d/)
    let br:RegExpMatchArray|null = null;
    let bl:RegExpMatchArray|null = null;

    if( left > -1 ) {
      bl = lines[bottom][left].match(/\d/)
    }
    if( right < lines.length ) {
      br = lines[bottom][right].match(/\d/)
    }

    if( bc ) {
      numbers.push(getFullNumber(bottom, c))
    }
    else {
      if( bl ) {
        numbers.push(getFullNumber(bottom,left))
      }
      if( br ) {
        numbers.push(getFullNumber(bottom,right))
      }
    }
  }
...
for( let l = 0; l < lines.length; l++) {
  ...
  for( let c = 0; c < lines.length; c++) {
    if( isSymbol( lines[l][c] )) {
      const foundNumbers = collectAdjacent(l, c)
      numbersByLine.push(...foundNumbers)
    }  
  }
  console.log(l, numbersByLine)
  numbers.push(...numbersByLine)
}

//console.log(numbers)
console.log(numbers.reduce( (n, s) => s+=n, 0))