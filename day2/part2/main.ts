...
games.forEach( g => {
  g.draws.forEach( draw => {
    if(draw.reds > g.maxRed) g.maxRed = draw.reds
    if(draw.greens > g.maxGreen) g.maxGreen = draw.greens
    if(draw.blues > g.maxBlue) g.maxBlue = draw.blues   
  })
  console.log(g)
  g.pow = g.maxBlue * g.maxGreen * g.maxRed
  console.log(g)
  sum += g.pow
})
console.log(sum)
