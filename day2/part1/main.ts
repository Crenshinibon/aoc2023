...
const parseDraws = (l):Draw[] => {
  const drawStrings = l.split(";")
  const result:Draw[] = [];
  
  drawStrings.forEach( ds => {
    const d:Draw = {reds:0, greens: 0, blues: 0}

    const colorStrings = ds.split(",")
    colorStrings.forEach( (cs:string) => {
      if(cs.endsWith("red")) {
        d.reds = parseInt(cs)
      }
      else if (cs.endsWith("blue")) {
        d.blues = parseInt(cs)
      }
      else if (cs.endsWith("green")) {
        d.greens = parseInt(cs)
      }
      else {
        console.log("weird")
      }
...
const parseInput = ():Game[] => {
  ...
  lines.forEach( l => {
    ...
    const gameNumber = l.substring(4, colonIndex)
    
    const draws = parseDraws(l.substring(colonIndex+1))
    const game:Game = {number: parseInt(gameNumber), draws: draws}
    result.push(game);
  })
  return result
}

const rule = (g:Game, maxRed:number, maxGreen:number, maxBlue:number):boolean => {
  for ( const d of g.draws) {
    if(d.reds > maxRed) return false
    if(d.blues > maxBlue) return false
    if(d.greens > maxGreen) return false
  }
  return true
}
